...
var transformSchema = function(schema) {
  return _.omit(
    _.extend(schema, {
      ...
      links: transformLinks(schema, schema.links || []),
 ...
 * @return {Array}
 */
var transformLinks = function(schema, links) {
  return _.map(links, function(link) {
    return transformLink(schema, link);
  });
};
 ...
 * @param {Array} link
 */
var transformLink = function(schema, link) {
  try {
    return _.omit(
      _.extend(link, {
        htmlID: _sanitizeHTMLID(schema.title + '-' + link.title),
        uri: buildHref(link.href, schema),
        curl: buildCurl(link, schema),
...
var buildHref = function(href, schema, withExampleData) {
  ...
  try {
    return _.reduce(matches, function (str, match) {
      ...
      return 'haha: ---> ' + stripped;
      //var definition = _resolver.get(schema.id + stripped);
 ...
 * @returns {String}
 */
var buildCurl = function (link, schema) {
  var page = {};
  var headers = {};
